...
var parse_rc_json = function parse_rc_json(stringified) {
    return _.isEmpty(stringified) ? { analysed_modules: [] } : JSON.parse(stringified);
};
var remove_report = function remove_report() {
    return fs.unlinkAsync(RC_REPORT);
};
var read_report = function read_report() {
    return fs.readFileAsync(RC_REPORT).then(function (data) {
        return remove_report().then(function () {
            return parse_rc_json(data);
        });
    });
};
var rubycritic = function rubycritic(paths) {
    return vile.spawn(RC_BIN, { args: RC_BASE_ARGS.concat(paths) }).then(read_report);
};
...
var vile_issues = function vile_issues(issue, config) {
    ...
    var issues = _.map(smells, function (smell) {
        if (smell_type(smell) == "duplicatecode") {
            ...
            var location = _.first(smell.locations);
            return vile.issue({
                type: vile.MAIN,
                path: issue.path,
                title: smell.context,
    ...
    if (_.has(issue, "rating") && _.has(config, "rating") && issue.rating.toLowerCase() > config.rating.toLowerCase()) {
        issues.push(vile.issue({
            type: vile.MAIN,
            path: issue.path,
...
var rc_issues_by_file = function rc_issues_by_file(cli_json) {
...
var punish = function punish(plugin_data) {
    ...
    return rubycritic(paths).then(rc_issues_by_file).then(function (files) {
